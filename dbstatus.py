...
queries={'tb_boundary_schoolcount':['select b2.id, count(case when s.active=2 then 1 else null end) as active_sch, count(distinct s.id) as all_sch from schools_boundary b,schools_boundary b1,schools_boundary b2,schools_institution s where s.boundary_id=b.id and b.parent_id=b1.id and b1.parent_id=b2.id group by b2.id',
'select b1.id , count(case when s.active=2 then 1 else null end) as active_sch, count(distinct s.id) as all_sch from schools_boundary b,schools_boundary b1,schools_boundary b2,schools_institution s where s.boundary_id=b.id and b.parent_id=b1.id and b1.parent_id=b2.id group by b1.id',
'select b.id, count(case when s.active=2 then 1 else null end) as active_sch, count(distinct s.id) as all_sch from schools_boundary b,schools_boundary b1,schools_boundary b2,schools_institution s where s.boundary_id=b.id and b.parent_id=b1.id and b1.parent_id=b2.id group by b.id'],
'tb_boundary_studentcount':['select b2.id, count(distinct s.id),count(distinct stu.id),count (case when c.gender =\'male\' then 1 else NULL end) as boys, count (case when c.gender =\'female\' then 1 else NULL end) as girls  from schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_student stu,schools_institution s,schools_boundary b,schools_boundary b1,schools_boundary b2, schools_child c where s.boundary_id=b.id and b.parent_id=b1.id and b1.parent_id=b2.id and s.active=2 and sg.institution_id=s.id and sg.active=2 and stusg.student_group_id=sg.id and stusg.active=2 and stusg.academic_id=122 and stusg.student_id=stu.id and stu.active=2 and stu.child_id = c.id group by b2.id',
'select b1.id, count(distinct s.id),count(distinct stu.id),count (case when c.gender =\'male\' then 1 else NULL end) as boys, count (case when c.gender =\'female\' then 1 else NULL end) as girls from schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_student stu,schools_institution s,schools_boundary b,schools_boundary b1,schools_boundary b2,schools_child c where s.boundary_id=b.id and b.parent_id=b1.id and b1.parent_id=b2.id and s.active=2 and sg.institution_id=s.id and sg.active=2 and stusg.student_group_id=sg.id and stusg.active=2 and stusg.academic_id=122 and stusg.student_id=stu.id and stu.active=2 and stu.child_id = c.id group by b1.id',
'select b.id, count(distinct s.id),count(distinct stu.id),count (case when c.gender =\'male\' then 1 else NULL end) as boys, count (case when c.gender =\'female\' then 1 else NULL end) as girls  from schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_student stu,schools_institution s,schools_boundary b,schools_boundary b1,schools_boundary b2, schools_child c where s.boundary_id=b.id and b.parent_id=b1.id and b1.parent_id=b2.id and s.active=2 and sg.institution_id=s.id and sg.active=2 and stusg.student_group_id=sg.id and stusg.active=2 and stusg.academic_id=122 and stusg.student_id=stu.id and stu.active=2 and stu.child_id = c.id group by b.id'],
'tb_boundary_totstudentcount':['select b2.id, count(distinct stusg.student_id)  from schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_institution s,schools_boundary b,schools_boundary b1,schools_boundary b2 where s.boundary_id=b.id and b.parent_id=b1.id and b1.parent_id=b2.id and sg.institution_id=s.id and stusg.student_group_id=sg.id group by b2.id',
'select b1.id, count(distinct stusg.student_id)  from schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_institution s,schools_boundary b,schools_boundary b1 where s.boundary_id=b.id and b.parent_id=b1.id and sg.institution_id=s.id and stusg.student_group_id=sg.id group by b1.id',
'select b.id, count(distinct stusg.student_id)  from schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_institution s,schools_boundary b where s.boundary_id=b.id and sg.institution_id=s.id and stusg.student_group_id=sg.id group by b.id'],
'tb_boundary_assessmentcount':['select distinct schoolsmapped.b2id,schoolsmapped.pname,schoolsmapped.assname,schoolsmapped.countmapped,schoolsassessed.countassessed,schoolsassessed.countstuassessed from (select distinct b2.id as b2id ,p.name as pname,ass.name as assname,count(distinct s.id) as countmapped from schools_assessment ass,schools_programme p,schools_studentgroup sg,schools_institution s,schools_boundary b,schools_boundary b1,schools_boundary b2,schools_assessment_studentgroup_association sgass where sgass.student_group_id=sg.id and sgass.assessment_id=ass.id and sg.active=2 and sg.institution_id=s.id and s.boundary_id=b.id and b.parent_id=b1.id and b1.parent_id=b2.id and  ass.programme_id=p.id and p.active=2 group by b2.id,p.name,ass.name)schoolsmapped left outer join ( select distinct b2.id as b2id,p.name as pname,ass.name as assname,count(distinct s.id) as countassessed, count(distinct stu.id) as countstuassessed from schools_answer se,schools_question q,schools_assessment ass,schools_programme p,schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_student stu,schools_institution s,schools_boundary b,schools_boundary b1,schools_boundary b2 where se.object_id=stu.id and stusg.student_id=stu.id and stusg.active=2 and stusg.academic_id=122 and stusg.student_group_id=sg.id and sg.active=2 and sg.institution_id=s.id and s.boundary_id=b.id and b.parent_id=b1.id and b1.parent_id=b2.id and se.question_id=q.id and q.assessment_id=ass.id and ass.programme_id=p.id and p.active=2 group by b2.id,p.name,ass.name)schoolsassessed on (schoolsmapped.b2id=schoolsassessed.b2id  and  schoolsmapped.pname=schoolsassessed.pname and schoolsmapped.assname=schoolsassessed.assname)',
...
'tb_schoolstudentcount':['select distinct s.id, count(distinct stu.id),count (case when c.gender =\'male\' then 1 else NULL end) as boys, count (case when c.gender =\'female\' then 1 else NULL end) as girls from schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_institution s,schools_student stu, schools_child c where stusg.student_id=stu.id and stu.active=2 and stusg.academic_id=122 and stusg.active=2 and stusg.student_group_id=sg.id and sg.active=2 and sg.institution_id=s.id and s.active=2 and stu.child_id = c.id group by s.id'],
'tb_classstudentcount':["select distinct sg.id,s.id,sg.name,sg.section,count(distinct stu.id),count (case when c.gender =\'male\' then 1 else NULL end) as boys, count (case when c.gender =\'female\' then 1 else NULL end) as girls from schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_institution s,schools_student stu, schools_child c where stusg.student_id=stu.id and stu.active=2 and stusg.academic_id=122 and stusg.active=2 and stusg.student_group_id=sg.id and sg.active=2 and sg.group_type='Class' and sg.institution_id=s.id and s.active=2 and stu.child_id = c.id group by sg.id,s.id,sg.name,sg.section"],
'tb_schoolassessmentcount':['select distinct s.id,p.name,ass.name,count(distinct stu.id) from schools_answer se,schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_institution s,schools_student stu,schools_question q,schools_assessment ass,schools_programme p where se.object_id=stu.id and stu.active=2 and stu.id=stusg.student_id and stusg.academic_id=122 and stusg.active=2 and stusg.student_group_id=sg.id and sg.active=2 and sg.institution_id=s.id and s.active=2 and se.question_id=q.id and q.assessment_id=ass.id and ass.programme_id=p.id and p.active=2 group by s.id,p.name,ass.name'],
'tb_classassessmentcount':["select distinct sg.id,s.id,sg.name,sg.section,p.name,ass.name,count(distinct stu.id) from schools_answer se,schools_student_studentgrouprelation stusg,schools_studentgroup sg,schools_institution s,schools_student stu,schools_question q,schools_assessment ass,schools_programme p where se.object_id=stu.id and stu.active=2 and stu.id=stusg.student_id and stusg.academic_id=122 and stusg.active=2 and stusg.student_group_id=sg.id and sg.active=2 and sg.group_type='Class' and sg.institution_id=s.id and s.active=2 and se.question_id=q.id and q.assessment_id=ass.id and ass.programme_id=p.id and p.active=2 group by sg.id,s.id,sg.name,sg.section,p.name,ass.name"]
}
